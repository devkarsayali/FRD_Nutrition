  ...
  return (
    <div className="bg-[#070a12] text-white min-h-[calc(100vh-80px)] py-6 sm:py-12 overflow-y-auto">
      <div className="container-custom max-w-6xl space-y-6 sm:space-y-8">
        {/* User Dashboard Welcome Banner Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0f172a] p-6 rounded-3xl border border-slate-800/90 shadow-2xl">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#f5b800] to-amber-500 text-slate-950 font-black text-xl flex items-center justify-center shadow-lg shrink-0">
              {(user.name || user.email || "U").charAt(0).toUpperCase()}
            </div>
            <div>
              <h2 className="text-xl font-bold font-heading text-white">
                Welcome back, {user.name || "FRD Athlete"}!
              </h2>
          ...
          <button
            ...
            className="self-start sm:self-auto px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-rose-400 hover:bg-rose-500/10 text-xs font-bold transition flex items-center gap-2 cursor-pointer"
          >
            <FiLogOut size={15} />
            <span>Log Out</span>
          </button>
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar bg-[#0f172a] p-2 rounded-2xl border border-slate-800 shadow-xl">
          <button
            onClick={() => setActiveTab("orders")}
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black tracking-wider transition whitespace-nowrap cursor-pointer ${
              activeTab === "orders"
          ...
          >
            ...
            <span>My Orders</span>
            {ordersList.length > 0 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] bg-slate-950/80 text-[#f5b800] font-black">
                {ordersList.length}
              </span>
            )}
          </button>
