...
export default function AdminDoughnutChart({
  ...
  return (
    <div className="bg-[#141813] border border-neutral-800 rounded-2xl p-4 sm:p-5 shadow-xl flex flex-col items-center justify-between space-y-4 relative overflow-hidden h-full">
      {/* Header Bar */}
      <div className="w-full flex items-center justify-between border-b border-neutral-800/80 pb-3">
        ...
        <span className="px-2 py-0.5 rounded-full bg-neutral-900 border border-neutral-800 text-[9px] font-extrabold text-lime-400 uppercase tracking-wider">
          Live Data
        </span>
      </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-2 pointer-events-none select-none">
            <AnimatePresence mode="wait">
              {activeSegment ? (
                ...
                >
                  <span
                    className="text-[11px] uppercase font-heading font-black tracking-wider block truncate max-w-[110px]"
                    style={{ color: activeSegment.color }}
                  >
                    {activeSegment.label}
                  </span>
                  <span
                    className="font-heading font-black text-2xl block leading-tight"
                    style={{ color: activeSegment.color }}
                  >
                    {activeSegment.value}
                  </span>
                  <span className="text-[9px] font-bold text-neutral-300 block uppercase tracking-wider">
                ...
                >
                  <span className="text-[9px] uppercase font-extrabold tracking-widest text-neutral-500 block">
                    {centerLabel}
                  </span>
                  <span className="font-heading font-black text-2xl sm:text-3xl text-white block leading-tight">
      ...
      {/* Horizontal Flex Pill Legend */}
      <div className="w-full flex flex-wrap items-center justify-center gap-2 pt-3 border-t border-neutral-800/80">
        {segments.map((seg, idx) => {
          const isHovered = hoveredIndex === idx;
          ...
          return (
            <button
              ...
              onMouseLeave={() => setHoveredIndex(null)}
              onClick={() => handleItemClick(seg)}
              className={`px-3 py-1 rounded-full border transition-all duration-200 cursor-pointer flex items-center gap-1.5 text-[11px] select-none ${
                isHovered
            ...
            >
              ...
              <span
                className="px-1.5 py-0.2 rounded-full text-[10px] font-black font-mono shrink-0 transition-colors"
                style={{
